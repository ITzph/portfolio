...
export class BlogsService {
  ...
  fetchAllBlogs() {
    return this.http.get<Blog[]>(`${environment.api}/${PORTFOLIO_ENDPOINTS.blogs}/all`);
  }

  fetchPublishedBlogs() {
    return this.http.get<Blog[]>(`${environment.api}/${PORTFOLIO_ENDPOINTS.blogs}`);
  }

  createBlog(blog: Partial<Blog>) {
    return this.http.post<Blog>(`${environment.api}/${PORTFOLIO_ENDPOINTS.blogs}`, blog);
  }

  fetchBlog(id: number) {
    return this.http.get<Blog>(`${environment.api}/${PORTFOLIO_ENDPOINTS.blogs}/${id}`);
  }

  updateBlog(id: number, blog: Partial<Blog>) {
    this.spinner.show('blogsSpinner');
    this.http
      .patch<Partial<Blog>>(`${environment.api}/${PORTFOLIO_ENDPOINTS.blogs}/${id}`, blog)
      .pipe(
        finalize(() => {
  ...
  deleteBlogById(id: number) {
    return this.http.delete<{ id: number }>(
      `${environment.api}/${PORTFOLIO_ENDPOINTS.blogs}/${id}`,
    );
  }
}
